...
function ResultGroup({title,items,onSelect}:{title:string;items:Array<{href:string;icon:React.ReactNode;title:string;subtitle:string}>;onSelect:()=>void}){
  ...
  return <section className="global-search-group-v2"><span>{title}</span>{items.map(item=><Link prefetch={false} key={item.href} href={item.href} onClick={onSelect}><i>{item.icon}</i><div><strong>{item.title}</strong><small>{item.subtitle}</small></div></Link>)}</section>;
}
