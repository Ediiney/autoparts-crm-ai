...
async function categoryId(
  ...
  const { data: existing } = await supabase.from("product_categories").select("id").eq("company_id",companyId).eq("normalized_name",normalized).maybeSingle();
  if(existing) return existing.id;
  const {data,error}=await supabase.from("product_categories").insert({company_id:companyId,name:category.trim(),normalized_name:normalized}).select("id").single();
...
export async function POST(request:Request) {
  try {
    ...
    if(!body.sku?.trim()||!body.name?.trim()) return NextResponse.json({error:"SKU e nome são obrigatórios."},{status:400});
    ...
    const {data:product,error}=await supabase.from("products").insert({
      ...
      source:"manual",
    }).select("id,sku,name").single();
    if(error) throw error;

    if(typeof body.price==="number"&&body.price>=0) {
      const {error:priceError}=await supabase.from("product_prices").insert({
        company_id:workspace.company.id,branch_id:workspace.branch?.id??null,product_id:product.id,price_type:"retail",price:body.price,source:"manual",
      });
      if(priceError) throw priceError;
    }

    if(typeof body.stock==="number"&&body.stock>=0) {
      let warehouseQuery=supabase.from("warehouses").select("id").eq("company_id",workspace.company.id).eq("active",true);
      ...
      if(warehouse) {
        const {error:stockError}=await supabase.from("product_inventory").insert({
          company_id:workspace.company.id,product_id:product.id,warehouse_id:warehouse.id,quantity:body.stock,reserved:0,
        });
    ...
    return NextResponse.json({error:error instanceof Error?error.message:"Não foi possível criar o produto."},{status:400});
  }
}
...
export async function PATCH(request:Request) {
  try {
    ...
    const catId=body.category!==undefined?await categoryId(supabase,workspace.company.id,body.category):undefined;
    ...
    }).eq("id",body.id).eq("company_id",workspace.company.id).select("id,sku,name").single();
    if(error) throw error;
    return NextResponse.json({product:data});
  } catch(error) {
    return NextResponse.json({error:error instanceof Error?error.message:"Não foi possível atualizar o produto."},{status:400});
  }
}
