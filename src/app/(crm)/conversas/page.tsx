...
export default async function ConversasPage({
  ...
  if (workspace.branch?.id) {
    conversationsQuery = conversationsQuery.eq("branch_id", workspace.branch.id);
  }

  const { data: conversations } = await conversationsQuery;
  const rows = conversations ?? [];
  const params = await searchParams;
  const activeId = rows.some((item) => item.id === params.id) ? params.id! : rows[0]?.id;

  const customerIds = [...new Set(
    rows.map((item) => item.customer_id).filter((id): id is string => Boolean(id)),
  )];

  const { data: customers } = customerIds.length
    ? await supabase
        .from("customers")
        .select("id,name,phone,whatsapp,email,timezone,created_at")
        .eq("company_id", companyId)
        .in("id", customerIds)
    : { data: [] as Array<{ id: string; name: string; phone: string | null; whatsapp: string | null; email: string | null; timezone: string | null; created_at: string }> };
  ...
  const [{ data: messages }, { data: aiInteraction }, { data: vehicles }] = activeId
    ? await Promise.all([
        ...
        supabase
          ...
          .select("part_name,vehicle,confidence,decision,missing_fields,created_at")
          .eq("company_id", companyId)
          .eq("conversation_id", activeId)
        ...
        activeCustomer
          ? supabase
              ...
              .select("id,brand,model,year,model_year,engine,version,transmission,plate")
              .eq("company_id", companyId)
  ...
  const displayTimezone = activeCustomer?.timezone || workspace.timezone;

  return (
    <>
      <div className="page-header">
        <div><div className="eyebrow">Atendimento</div><h1>Conversas</h1><p>Inbox da filial com histórico, contexto do cliente e identificação de peças.</p></div>
      ...
      {rows.length ? (
        <div className="inbox-layout">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="conversation-meta">
                      <time>{formatTimeInTimezone(conversation.last_message_at || conversation.created_at, customer?.timezone || workspace.timezone)}</time>
                    </div>
                  </Link>
                );
              })}
          ...
          <section className="chat-panel">
            <header className="chat-header">
              <div className="chat-contact">
                <Avatar initials={initials(activeCustomer?.name ?? "Cliente")} tone="green" />
                <div>
                  <strong>{activeCustomer?.name ?? "Cliente"}</strong>
              ...
              {(messages ?? []).length ? (messages ?? []).map((message) => (
                <div className={`message-row ${message.sender_type === "customer" ? "customer" : message.sender_type === "agent" ? "agent" : "ai"}`} key={message.id}>
                  <div className="message-bubble">
                    {message.content}
                    <small>{formatTimeInTimezone(message.created_at, displayTimezone)} · {message.sender_type === "customer" ? "cliente" : message.sender_type === "agent" ? "atendente" : "automação"}</small>
                  </div>
                </div>
              )) : (
              ...
              {aiInteraction ? (
                <div className="ai-note">
                  ...
                  <div>
                    ...
                    <p>{aiInteraction.part_name || "Peça não identificada"} · {aiInteraction.decision}{aiInteraction.missing_fields.length ? ` · faltando: ${aiInteraction.missing_fields.join(", ")}` : ""}</p>
                  </div>
                </div>
              ) : null}
            </div>
            ...
            <div className="chat-composer">
              <div className="composer-box">
                ...
                <div className="composer-actions">
                  <div className="composer-left"><Button variant="ghost" icon={<FileText size={14} />}>Criar orçamento</Button></div>
          ...
          <aside className="contact-panel">
            <div className="contact-profile">
              <Avatar initials={initials(activeCustomer?.name ?? "Cliente")} tone="green" />
              <h3>{activeCustomer?.name ?? "Cliente"}</h3>
              <p>{activeCustomer?.email || "Sem e-mail"}</p>
              <StatusBadge tone={tone(activeConversation?.status ?? "open")}>{statusLabel[activeConversation?.status ?? "open"]}</StatusBadge>
            </div>
            ...
            <div className="info-section">
              ...
              {vehicle ? (
                ...
                <div className="vehicle-card"><strong>Veículo não cadastrado</strong><span>Os dados podem ser coletados durante a conversa.</span></div>
              )}
            </div>
        ...
        <div className="surface-card empty-state"><MessageCircleMore size={34} /><strong>Nenhuma conversa registrada</strong><p>O primeiro atendimento da filial aparecerá aqui.</p></div>
      )}
    </>
  );
}
