...
export default async function ProductDetailPage({params}:{params:Promise<{id:string}>}){
  ...
  const currentPrices=(pricesResult.data??[]).filter(p=>p.branch_id===branchId||p.branch_id===null);
  ...
  return (
    <div className="product-detail-v2">
      ...
      <div className="product-detail-v2-head">
        <div className="product-detail-v2-identity">
          <div className="product-detail-v2-media">
            {primaryMedia?<img src={primaryMedia.url} alt={primaryMedia.alt_text||product.name}/>:<Package size={32}/>}
          </div>
          <div>
      ...
      <div className="detail-metrics-v2">
        <Metric icon={<CircleDollarSign size={17}/>} label="Preço atual" value={currentPrice?money(Number(currentPrice.price)):"Sem preço"} sub={workspace.branch?.name||"Empresa"}/>
      ...
      <div className="detail-grid-v2">
        <section className="panel-v2">
          ...
          <div className="definition-grid-v2">
            <Def label="SKU" value={product.sku}/>
            <Def label="Código original" value={product.original_code}/>
            <Def label="Fabricante" value={product.manufacturer}/>
            <Def label="Marca" value={product.brand}/>
            <Def label="Código de barras" value={product.barcode}/>
            <Def label="Origem" value={product.source}/>
          </div>
          {product.description?<div className="description-v2"><span>Descrição</span><p>{product.description}</p></div>:null}
        </section>

        <section className="panel-v2">
          <div className="panel-v2-title"><div><h2>Preço por filial</h2><p>Histórico e tabela comercial.</p></div></div>
          <div className="mini-table-v2">
            {(pricesResult.data??[]).length?(pricesResult.data??[]).slice(0,8).map(price=>(
              <div className="mini-table-v2-row" key={price.id}>
            ...
            )):<EmptyLine text="Nenhum preço cadastrado."/>}
          </div>
        </section>
        ...
        <section className="panel-v2 span-2">
          ...
          {(appsResult.data??[]).length?(
            <div className="application-grid-v2">
              {(appsResult.data??[]).map(app=>(
                <div className="application-v2" key={app.id}>
          ...
          ):<EmptyLine text="Nenhuma aplicação cadastrada."/>}
        </section>
...
function Def({label,value}:{label:string;value:string|null}){
  return <div className="definition-v2"><span>{label}</span><strong>{value||"—"}</strong></div>;
}
